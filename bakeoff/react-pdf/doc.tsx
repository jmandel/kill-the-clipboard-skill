/**
 * doc.tsx — semantic document builder on top of @react-pdf/renderer.
 *
 * Exposes theme-aware semantic components (title, section, para, pullQuote,
 * callout, kvPanel, table, badge, pageFooter, page, renderDoc). Entry scripts
 * compose these; no layout math leaks out of this module.
 */
...
export const PROVENANCE = "Shared by the patient via SMART Health Link — June 10, 2026";

/** Fixed footer on every page: provenance line + Page N of M. */
export function pageFooter(t: Theme) {
  return (
    <View
      key="footer"
      fixed
    ...
    >
      <Text style={{ fontFamily: t.font, fontSize: 7.5, color: t.muted }}>{PROVENANCE}</Text>
      <Text
...
export function page(
  ...
  opts?: { orientation?: "portrait" | "landscape"; key?: string },
) {
  return (
    <Page
      key={opts?.key}
...
export async function renderDoc(
  ...
  registerFonts();
  const docEl = (
    <Document title={meta.title} author={meta.author} producer="react-pdf bakeoff" creator="react-pdf bakeoff">
      {pages}
    </Document>
  );
