...
5. **KTC defaults**: exp = 24h, maxUses = 5, manual re-arm (PATCH + client-side QR rebuild). Grace and flexibility over crushing users in short windows. AMENDED 2026-06-12: **`exp: null` = never expires**, first-class and symmetric with `maxUses: null` (schema migrated in place via an idempotent boot-time rebuild — production not wiped). `--exp-hours never` on create-shl/re-arm; the payload then omits `exp` (valid base SHL, NOT KTC-conformant — clinic check-in links keep a finite expiry); owner page shows "Never expires", re-arm picker gains "no expiration". Retention: the sweeper exempts null-exp links — destroy is their retention event.
