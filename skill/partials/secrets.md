...
**Handing off is simple — don't overthink it.** When the link is ready, read
`owner-link.txt` and present the owner page URL to the patient as a clickable link,
along with `qr.png` however your platform best shows or delivers a file. That's the
intended delivery — reading the file to relay it to the patient is the point of the
file, and this conversation is the patient's own; the secret appearing in it is fine.
If the patient asks for the link again later, give it again.
