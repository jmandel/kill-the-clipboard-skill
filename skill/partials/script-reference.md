...
| `create-shl.ts` | `bun create-shl.ts --bundle bundle.json --label "..." [--exp-hours 24] [--max-uses 5] [--flag U] -o ./shl-out/` | Encrypts + uploads + registers the link. stdout includes `handoffMarkdown` — the closing message to paste verbatim (exact contract per Step 7); artifacts to files in `-o` dir. |
